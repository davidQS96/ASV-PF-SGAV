...
class Car:
    ...
    def __init__(self, circuit, screen, prgmStatus, color, radius = 5, initSpeed = 1):
    ...
    def decideNextMovement(self):
        ...
        print(commandNum, "linea 379 graphics")
        ...
        if len(command) == 1: # (action)
            if command[0] == 0: #detencion
                time.sleep(10)
                print("10 segundos ya pasaron")

                while self.prgmStatus.nextCommandNum != 6: #Puesta en marcha despues de detencion
                    continue
            ...
            newSpeed = self.speed + round(0.01 * (1 - 2 * velUp), 2) # (1 - 2 * velUp) unicamente tiene valores +-1

            if newSpeed < 0.005: #Si la velocidad no permite un funcionamiento adecuado del carro
